  ...
  return (
    <div className="app-shell" dir="rtl">
      ...
      <aside className="desktop-sidebar" aria-label="التنقل الرئيسي / Main navigation">
        ...
        <button type="button" className={activeTab === "review" ? "sidebar-link is-active" : "sidebar-link"} onClick={() => { setActiveTab("review"); if (selectedBank === "ycb") setReviewPreview("accountStatement"); }}><ClipboardList size={18} /><span>{selectedBank === "ycb" ? "معاينة البيان" : "المعاينة"}<small>{selectedBank === "ycb" ? "Statement Preview" : "Preview & Print"}</small></span></button>
      ...
      {activeTab === "review" && <section className="panel review-panel">
        ...
        <div className="actions document-actions"><button type="button" className="secondary-button" onClick={() => void saveStatementHistory()} disabled={createHistoryMutation.isPending || updateHistoryMutation.isPending}><Database size={17} /> {editingHistoryId ? "Update Saved Statement" : "Save to Statement History"}</button><button type="button" onClick={() => selectedBank === "ycb" || selectedBank === "tadhamon" ? setReviewPreview("accountStatement") : setReviewPreview("accountStatus")}><FileText size={17} /> {selectedBank === "ycb" ? "View Approved YCB Statement" : selectedBank === "tadhamon" ? "View Approved Tadhamon Statement" : "View Account Status Statement"}</button><button type="button" className="preview-button" onClick={() => selectedBank === "ycb" ? setReviewPreview("accountStatement") : setReviewPreview("accountStatement")}><FileText size={17} /> View Account Statement</button><button type="button" className="unified-print-button" onClick={() => selectedBank === "ycb" ? setReviewPreview("accountStatement") : printDocument("unified")}><Printer size={17} /> {selectedBank === "ycb" ? "Open Approved YCB Statement" : selectedBank === "tadhamon" ? "Open Approved Tadhamon Statement" : "Print Unified PDF (Statement + Status)"}</button><button type="button" onClick={() => printDocument("accountStatus")}><Printer size={17} /> {selectedBank === "ycb" ? "Print Approved YCB PDF" : selectedBank === "tadhamon" ? "Print Tadhamon Status / Save PDF" : "Print Account Status / Save PDF"}</button><button type="button" className="preview-button" onClick={() => selectedBank === "ycb" ? setReviewPreview("accountStatement") : printDocument("accountStatement")}><Printer size={17} /> Print Account Statement / Save PDF</button><button type="button" disabled={downloadingDocument === "accountStatus"} onClick={() => void downloadPdf("accountStatus")}><Download size={17} /> {downloadingDocument === "accountStatus" ? "Opening print…" : selectedBank === "ycb" ? "Print / Save Approved YCB PDF" : selectedBank === "tadhamon" ? "Print / Save Tadhamon Status PDF" : "Print / Save Account Status PDF"}</button><button type="button" className="preview-button" disabled={selectedBank === "ycb" || downloadingDocument === "accountStatement"} onClick={() => void downloadPdf("accountStatement")}><Download size={17} /> {downloadingDocument === "accountStatement" ? "Opening print…" : "Print / Save Account Statement PDF"}</button><button type="button" className="secondary-button" onClick={downloadSessionJson}><RefreshCcw size={17} /> Download JSON Session</button></div>
