...
const css = `@page{size:A4;margin:0}:root{--yellow:#fff200;--red:#e53218;--line:#171717;--blue:#9bd5e4}*{box-sizing:border-box}html,body{margin:0;padding:0;background:#e8e8e8;color:#111;font-family:Arial,Tahoma,sans-serif}body{padding:12px 0}.page{width:210mm;height:297mm;min-height:0;margin:0 auto 12px;padding:3mm 5mm;background:#fff;box-shadow:0 0 8px #999;overflow:hidden;page-break-after:always;break-after:page}.page:last-child{page-break-after:auto;break-after:auto}.top-info{display:grid;grid-template-columns:13.5% 13% 19% 11.2% 11.2% 13.6% 18.5%;grid-template-rows:8mm;width:100%;height:8mm;margin:0 auto 1mm;align-items:stretch;position:relative;top:0}.info-box{border:1.1px solid var(--line);border-right:0;text-align:center;background:linear-gradient(#f4da4e,#fff7a7);height:8mm;min-height:8mm;max-height:8mm;overflow:hidden}.info-box:last-child{border-right:1.1px solid var(--line)}.info-label{display:block;font-weight:700;font-size:6.2pt;padding:.8mm .5mm .5mm;border-bottom:1px solid var(--line);line-height:1.02}.ar{display:block;font-size:5.8pt;font-weight:700}.info-value{display:block;height:3.4mm;padding:.1mm;font-size:6.2pt;font-weight:700;white-space:nowrap;overflow:hidden}.statement{width:100%;margin:0 auto;border-collapse:collapse;table-layout:fixed;direction:ltr;font-size:6.8pt}.statement th,.statement td{border:1px solid var(--line);padding:.35mm .45mm;text-align:center;vertical-align:middle;line-height:1.05;min-height:5.05mm;height:auto}.statement thead th{background:var(--red);color:#fff;font-weight:700;height:6mm;min-height:6mm;font-size:5.6pt}.statement thead .ar{font-size:5.5pt}.statement col.date{width:12%}.statement col.ref{width:12%}.statement col.desc{width:24%}.statement col.debit{width:10.5%}.statement col.credit{width:10.5%}.statement col.balance{width:14%}.statement col.remarks{width:17%}.statement tbody tr{background:var(--yellow);height:var(--quick-row-height,5.05mm)}.statement tbody tr:nth-child(5n){background:var(--yellow)}.statement tr.deposit td{background:var(--yellow)}.statement tr.highlight td{background:var(--highlight-color,#fff)!important;color:#111!important}.statement td.desc-cell{font-weight:600;font-size:6.7pt;line-height:1.25;white-space:normal;overflow-wrap:anywhere;word-break:break-word;min-height:5.05mm;height:auto}.bottom-date{display:flex;width:100%;margin:.8mm 0 0;border:1.1px solid var(--line);font-weight:700;direction:ltr;font-size:5.5pt;white-space:nowrap}.bottom-date>*{padding:.15mm .8mm;min-height:3.2mm;text-align:center}.bottom-date .date-label{width:42%;background:#e9df78}.bottom-date .date-value{width:58%;background:#fff}.footer{margin:1.2mm 0 0;width:100%;display:flex;gap:10mm;align-items:flex-start;direction:ltr}.identity-block{width:100%}.closing-row{display:flex;justify-content:space-between;align-items:flex-start;margin-top:1.2mm}.closing-row>div:first-child{width:26.5%;flex:0 0 26.5%}.qr-block{width:30mm;margin-left:auto;text-align:center;color:#111}.qr-name{display:block;font-size:6pt;font-weight:700;line-height:1.05;margin-bottom:.8mm}.qr-image{display:block;width:25mm;height:25mm;margin:0 auto;image-rendering:pixelated}.name-box{background:linear-gradient(#9bdae9,#64b5c9);border:1px solid #5599a8;text-align:center;padding:.55mm 1mm;font-size:6.4pt;font-weight:700;min-height:8mm;white-space:nowrap;direction:ltr}.name-ar{display:block;font-size:6.4pt;margin-top:.15mm;direction:rtl}.total-row{position:relative;display:block;direction:ltr;margin:0;width:100%;font-size:8.5pt}.total-row>div{border:1.1px solid var(--line);min-height:7mm;display:flex;align-items:center;justify-content:center;font-weight:700}.total-combined{width:100%;background:#fff200;font-size:8.5pt;white-space:nowrap;padding:0 1mm;direction:ltr}.time{position:absolute;left:calc(100% + 2mm);top:0;width:max-content;color:#9b1b1b;font-size:7pt;background:#fff;white-space:nowrap;padding:1.8mm 1mm}@media print{html,body{background:#fff;padding:0}.page{margin:0;box-shadow:none}}`;

function header(profile: TadhamonFastStatementProfile, style = "") {
  return `<section class="top-info"${style}><div class="info-box"><span class="info-label">Account Opening Date<span class="ar">تاريخ فتح الحساب</span></span><span class="info-value">${esc(profile.openingDate || profile.customerSince || "—")}</span></div><div class="info-box" style="grid-column:2 / span 4"><span class="info-label">Account Number<span class="ar">رقم الحساب</span></span><span class="info-value">${esc(profile.accountNumber)}</span></div><div class="info-box" style="grid-column:6 / span 2"><span class="info-label">Opening Balance<span class="ar">الرصيد الافتتاحي</span></span><span class="info-value">${money(profile.openingBalance)}</span></div></section>`;
}
...
export function renderTadhamonFastStatement(profile: TadhamonFastStatementProfile, transactions: YcbStatementTransaction[], highlights: Record<string, string> = {}, rowsPerPage = 35) {
  ...
  const pageMarkup = Array.from({ length: pages }, (_, pageIndex) => {
    ...
    const rowHeight = isKarimi || isYcb ? 5.05 : 5.05;
