...
describe("reference document previews", () => {
  ...
  it("allows the Kuraimi statement description to display up to four lines when requested", () => {
    const html = renderStatementPreview({ headerUri: "/header.png", qrUri: "/qr.png", customerName: "Client", accountNumber: "1", momaizNo: "2", branchName: "HADDAH", currency: "USD", issueDate: "15/08/2026", periodStart: "01/08/2026", periodEnd: "15/08/2026", statementReference: "BAK-ACCT-20260801-0001", pageNumber: 1, pageCount: 1, barcodeUri: "/barcode.svg", barcodeLabel: "REF P1 of 1", closing: 20, descriptionMaxLines: 4, transactions: [{ date: "15/08/2026", description: "A deliberately long transaction description that should remain readable across four lines in the Kuraimi statement", operationNumber: "FT260815AAA", debit: 0, credit: 20, balance: 20 }] });
    expect(html).toContain("-webkit-line-clamp:4");
    expect(html).toContain("max-height:11.4mm");
    expect(html).toContain("max-height:11.2mm");
    expect(html).toContain("height:auto;min-height:10mm;max-height:none");
    expect(html).toContain("visibility:visible;opacity:1");
  });
});
