...
export function renderTadhamonFastStatement(profile: TadhamonFastStatementProfile, transactions: YcbStatementTransaction[], highlights: Record<string, string> = {}, rowsPerPage = 35) {
  ...
  const pageMarkup = Array.from({ length: pages }, (_, pageIndex) => {
    ...
    const expandRows = profile.bankName === "بنك الكريمي" || profile.bankName === "البنك التجاري اليمني";
    const rowHeight = expandRows ? Math.max(5.05, Math.min(16, 235 / Math.max(pageRows.length, 1))) : 5.05;
    return `<main class="page">${pageIndex === 0 ? header(profile) : ""}<table class="statement" style="--quick-row-height:${rowHeight}mm"><colgroup><col class="date"><col class="ref"><col class="desc"><col class="debit"><col class="credit"><col class="balance"><col class="remarks"></colgroup>${tableHeader()}<tbody>${pageRows.map((item) => row(item, highlights)).join("")}</tbody></table>${isLast ? closing(profile, profile.qrUri || "") : ""}</main>`;
  }).join("");
