  ...
  return (
    <div key={selectedBank} className="app-shell" dir="rtl">
      ...
      {activeTab === "fastStatement" && selectedBank === "tadhamon" && <section className="panel print-preview-panel" dir="rtl">
        ...
        <div className="actions"><button type="button" className="secondary-button" onClick={() => highlightDepositsWhite()}><RefreshCcw size={17} /> تلوين الإيداعات بالأبيض / White Deposits</button><label className="computed-field"><span>حد مبلغ الإيداع / Minimum Deposit</span><input className="transaction-edit-input" type="number" min="0" step="0.01" dir="ltr" value={fastMinimumDeposit} onChange={(event) => setFastMinimumDeposit(event.target.value)} placeholder="100.00" /></label><button type="button" className="secondary-button" onClick={() => { const minimum = money(fastMinimumDeposit); if (minimum > 0) highlightDepositsWhite(minimum); }} disabled={!fastMinimumDeposit.trim() || money(fastMinimumDeposit) <= 0}><RefreshCcw size={17} /> تحديث حسب المبلغ / Apply Minimum</button><div className="keyword-color-controls" style={{ display: "flex", gap: "10px", alignItems: "end", flexWrap: "wrap", width: "100%", padding: "12px", border: "1px solid #dbe3ec", borderRadius: "8px", background: "#f8fafc" }}><label className="computed-field"><span>كلمة البحث داخل الوصف / Keyword</span><input className="transaction-edit-input" value={fastKeyword} onChange={(event) => setFastKeyword(event.target.value)} placeholder="مثال: إيداع" /></label><label className="computed-field"><span>لون الوصف المطابق / Match Color</span><select className="transaction-edit-input" value={fastKeywordColor} onChange={(event) => setFastKeywordColor(event.target.value)}><option value="#e5e7eb">أسود / Black</option><option value="#dcfce7">أخضر / Green</option><option value="#dbeafe">أزرق / Blue</option><option value="#fef9c3">أصفر / Yellow</option><option value="#fee2e2">أحمر / Red</option></select></label><button type="button" className="secondary-button" onClick={applyFastKeywordColor} disabled={!fastKeyword.trim()}>تطبيق على الأوصاف المطابقة / Apply</button><small style={{ color: "#475569", width: "100%" }}>يتم تغيير لون الصفوف التي يحتوي وصفها على الكلمة فقط، مع إبقاء خط الوصف أسوداً. / Matching rows only; text stays black.</small></div><button type="button" className="preview-button" onClick={() => openPrintWindow(tadhamonFastStatementHtml, "Tadhamon Bank — Quick Account Statement")}><FileText size={17} /> معاينة / Preview</button><button type="button" onClick={() => void downloadPdf("accountStatement")}><Printer size={17} /> طباعة / Print</button><button type="button" className="unified-print-button" onClick={() => printDocument("unifiedAll")}><Printer size={17} /> طباعة موحدة / Unified Print</button></div>
