...
export default function YcbStatementWorkspace({ profile, transactions, onBack }: Props) {
  ...
  const selectedColor = selectedReference ? rowHighlights[selectedReference] || "#FEF08A" : "#FEF08A";
  const updateSelectedColor = (color: string) => {
    if (!selectedReference) return;
    setRowHighlights((current) => ({ ...current, [selectedReference]: color }));
  };
  const clearSelectedColor = () => {
    if (!selectedReference) return;
    setRowHighlights((current) => { const next = { ...current }; delete next[selectedReference]; return next; });
  };
  return <main className="bank-workspace" dir="ltr">
    ...
    <section className="panel"><div className="panel-heading"><div><h2>تخصيص ألوان الحركات</h2><p className="hint">اختر حركة ثم حدد لون التمييز. اللون يظهر في المعاينة ويحفظ عند الطباعة أو حفظ PDF.</p></div></div><div style={{ display: "flex", gap: "10px", alignItems: "center", flexWrap: "wrap" }}><select value={selectedReference} onChange={(event) => setSelectedReference(event.target.value)} aria-label="Select transaction"><option value="">اختر حركة لتلوينها</option>{transactions.map((row) => <option key={row.reference} value={row.reference}>{row.reference} · {row.description}</option>)}</select><input aria-label="Custom highlight color" type="color" value={selectedColor} disabled={!selectedReference} onChange={(event) => updateSelectedColor(event.target.value)} /><button type="button" className="secondary-button" disabled={!selectedReference} onClick={clearSelectedColor}>مسح اللون</button><span className="hint">اللون الافتراضي: أصفر فاتح</span></div></section>
