...
export function renderOriginalTadhamonStatementPage(profile: YcbStatementProfile, transactions: YcbStatementTransaction[], pageNumber: number, pageCount: number, qrUri = "", barcodeUri = "", rowHighlights: Record<string, string> = {}) {
  ...
  const summary = `<section class="summary"><div class="sum"><div class="label">Opening Balance</div><strong>${opening}</strong></div><div class="sum"><div class="label">Total Credit</div><strong>${credit}</strong></div><div class="sum"><div class="label">Total Debit</div><strong>${debit}</strong></div><div class="sum"><div class="label">Closing Balance</div><strong>${closing}</strong></div><div class="code-sum"><img class="summary-qr" src="${escapeHtml(qrUri)}" alt="QR statement data"></div></section>`;
...
export function renderTadhamonStatementPages(profile: YcbStatementProfile, transactions: YcbStatementTransaction[], qrSources: string[] = [], barcodeSources: string[] = [], rowHighlights: Record<string, string> = {}) {
  const pageSize = 15;
  const visibleTransactions = transactions.slice(0, pageSize);
  return renderOriginalTadhamonStatementPage({ ...profile, pageNumber: 1, pageCount: 1 }, visibleTransactions, 1, 1, qrSources[0], barcodeSources[0], rowHighlights);
}
