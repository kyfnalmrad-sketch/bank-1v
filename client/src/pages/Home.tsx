...
function AuthenticatedHome({ user, logout }: { user: { name?: string | null; email?: string | null }; logout: () => Promise<void> }) {
  ...
  }, [barcodeSources, documentClient, documentPrintDate, documentPeriodEnd, quickHighlightConfig.depositColor, quickHighlightConfig.label, reportedClosing, reportedTotalCredit, reportedTotalDebit, selectedBank, statementQrSources, statementReference, statementRows, fastHighlightColors, fastKeyword, fastKeywordColor, fastRowsPerPage, ycbClient.customerSince, ycbStatementProfile]);
  ...
  return (
    <div key={selectedBank} className="app-shell" dir="rtl">
      ...
      <aside className="desktop-sidebar" aria-label="التنقل الرئيسي / Main navigation">
        ...
        {selectedBank && <button type="button" className={activeTab === "fastStatement" ? "sidebar-link is-active" : "sidebar-link"} onClick={() => setActiveTab("fastStatement")}><FileText size={18} /><span>طبعة كشف حساب سريع<small>Quick Statement Print</small></span></button>}
