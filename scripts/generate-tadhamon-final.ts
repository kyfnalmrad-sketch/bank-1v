...
const overrides = `<style>.page>.top{height:68mm!important}.top [style*="border-top:1px solid #d1d7dc"]{border-top:0!important;padding-top:0!important}.identity-field{border:1px solid #b7c1c8;padding:1.2mm 1.5mm;margin-bottom:1mm;background:rgba(255,255,255,.9)}.identity-field .field-label{display:block;font-weight:800;font-style:italic;text-transform:uppercase}.identity-field .field-value{display:block;margin-top:.7mm}.address-grid{display:block;margin-top:1mm}.address-grid .address-field{min-width:0;border:0;border-bottom:1px solid #b7c1c8;padding:1.2mm 0;background:transparent;white-space:nowrap;overflow:hidden;text-overflow:ellipsis}.address-grid .field-label{display:block;font-weight:800;font-style:italic;text-transform:uppercase}.address-grid .field-value{display:block;margin-top:.7mm}.summary{grid-template-columns:1fr 27mm!important;height:auto!important;min-height:24mm!important}.summary .sum{grid-column:1;display:grid;grid-template-columns:1fr auto;align-items:center;padding:1.4mm 2mm;border-right:0;border-bottom:1px solid #b2bec7}.summary .sum:last-of-type{border-bottom:0}.summary .code-sum{grid-column:2;grid-row:1 / span 4}.code-sum{display:flex;align-items:center;justify-content:center;gap:1mm;padding:1mm;border-left:1px solid #b2bec7}.summary-qr{width:22mm;height:22mm;object-fit:contain}.row{grid-template-columns:13% 14% 43% 8% 8% 14%!important}.page>.summary{margin-top:3mm!important}.page>.table{margin-top:3mm!important}.page>.notes{margin-top:3mm!important}.row:nth-child(odd):not(.head):not(.total) .cell{background:#E2E6EA}.row.credit-row .cell{background:#E8F8F5;color:#1B365D}@media print{.page{margin:0!important}}</style>`;
const renderPage = async (pageRows: typeof rows, pageNumber: number, pageCount: number) => {
  ...
  const summary = `<section class="summary"><div class="sum"><div class="label">Opening Balance</div><strong>${money(profile.openingBalance)}</strong></div><div class="sum"><div class="label">Total Credit</div><strong>${money(profile.totalCredit)}</strong></div><div class="sum"><div class="label">Total Debit</div><strong>${money(profile.totalDebit)}</strong></div><div class="sum"><div class="label">Closing Balance</div><strong>${money(pageRows.at(-1)?.balance ?? profile.closingBalance)}</strong></div><div class="code-sum"><img class="summary-qr" src="${qrUri}" alt="QR statement data"></div></section>`;
  ...
  let html = template.replace(/<div style="font-weight:800;font-style:italic;text-transform:uppercase">Customer Name<\/div><div style="margin-top:1mm">Arafat Ali Saleh Dilla<\/div>/, `<div class="identity-field"><span class="field-label">Customer Name:</span><span class="field-value">${esc(profile.customerName)}</span></div><div class="identity-field"><span class="field-label">Date of Birth:</span><span class="field-value">${esc(profile.dateOfBirth)}</span></div><div class="identity-field"><span class="field-label">Place of Birth:</span><span class="field-value">${esc(profile.placeOfBirth)}</span></div>`).replace(/<div style="border-top:1px solid #d1d7dc;margin-top:2.2mm;padding-top:1.8mm;font-weight:800;font-style:italic;text-transform:uppercase">Address<\/div>/, "").replace(/<div class="address-line">[\s\S]*?<\/div><\/div><div style="padding:4mm 2mm;text-align:center/, `<div class="address-grid"><div class="address-field"><span class="field-label">Address:</span><span class="field-value">${esc(profile.address)}</span></div></div></div><div style="padding:4mm 2mm;text-align:center`).replace("AL-ZUBAIRI", esc(profile.branchName)).replace("101-840-21102-326491-000", esc(profile.accountNumber)).replace("05-Feb-2025", esc(profile.periodStart)).replace("24-Jun-2025", esc(profile.periodEnd)).replace(/(<b>Currency:<\/b>\s*)USD(\s*&nbsp;\s*&nbsp;\s*<b>Page:<\/b>\s*)1 of 1/, `$1${esc(profile.currency)}$2${pageNumber} of ${pageCount}`).replace(/<div class="screenbar">[\s\S]*?<\/div>/, "").replace("Tadhamon-STMT-2025-001", esc(profile.statementReference)).replace(/<div style="font-size:7.5pt;color:#425766;margin-top:1.4mm">Tadhamon Bank<\/div>/, "").replace(/border-top:1px solid #d1d7dc;/g, "").replace(/<img class="address-qr"[^>]*>/g, "").replace(/(<img class="title-pdf417" src=")[^"]*(")/, `$1${barcodeUri}$2`).replace(/<footer class="footer">[\s\S]*?<\/footer>/, "");
