...
export function BankSelector({ onSelect, onLogout }: { onSelect: (bank: "karimi" | "ycb") => void; onLogout: () => void }) {
  return <main className="bank-selector" dir="rtl"><section className="bank-selector-card"><div className="system-mark"><FileText size={30} /></div><p className="eyebrow">منصة إصدار ومراجعة الكشوف</p><h1>اختر مساحة العمل</h1><p className="hint">كل بنك يعمل في مساحة مستقلة. بنك التضامن ظاهر كمساحة قادمة، ولن تُخلط بياناته مع البنوك الحالية.</p><div className="bank-choice-grid"><button type="button" onClick={() => onSelect("karimi")}><strong>بنك الكريمي</strong><span>نظام الكشوف الحالي</span></button><button type="button" onClick={() => onSelect("ycb")}><strong>بنك اليمن التجاري</strong><span>القالب الرسمي ومدخلات YCB</span></button><button type="button" className="bank-choice-coming" disabled><strong>بنك التضامن</strong><span>مساحة قادمة — تُفعّل بعد استلام النماذج الرسمية</span><em>قيد التجهيز</em></button></div><section className="bank-data-table-wrap"><h2>فصل البيانات والاستعادة</h2><p className="hint">كل بنك يستعيد Snapshot وHistory والعمليات إلى مساره فقط.</p><table className="bank-data-table"><thead><tr><th>البنك</th><th>Snapshot</th><th>History</th><th>Excel / العمليات</th><th>الحالة</th></tr></thead><tbody><tr><td><strong>الكريمي</strong></td><td>مساحة Karimi</td><td>سجلات Karimi</td><td>محرك الكريمي</td><td>مفعّل</td></tr><tr><td><strong>اليمن التجاري</strong></td><td>مساحة YCB</td><td>سجلات YCB</td><td>محرك مستقل</td><td>مفعّل</td></tr><tr><td><strong>التضامن</strong></td><td>مساحة مستقلة لاحقًا</td><td>سجلات مستقلة لاحقًا</td><td>بانتظار النموذج الرسمي</td><td>قيد التجهيز</td></tr></tbody></table></section><button type="button" className="secondary-button" onClick={onLogout}>تسجيل الخروج</button></section></main>;
}
