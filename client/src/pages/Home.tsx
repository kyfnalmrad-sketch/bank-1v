...
function AuthenticatedHome({ user, logout }: { user: { name?: string | null; email?: string | null }; logout: () => Promise<void> }) {
  ...
  const [fastKeywordColor, setFastKeywordColor] = useState(quickHighlightConfig.keywordColor);
  ...
  useEffect(() => {
    setFastKeywordColor(quickHighlightConfig.keywordColor);
  }, [quickHighlightConfig.keywordColor]);
  ...
  const highlightDepositsWhite = (minimum?: number) => {
    ...
    transactions.forEach((transaction) => {
      if (transaction.credit > 0 && (minimum === undefined || transaction.credit >= minimum)) next[transaction.rowNumber] = quickHighlightConfig.depositColor;
    });
    setFastHighlightColors(next);
    setImportNote(minimum === undefined ? `تم تلوين جميع عمليات الإيداع بلون ${quickHighlightConfig.label}.` : `تم تلوين الإيداعات من ${formatMoney(minimum)} فأعلى بلون ${quickHighlightConfig.label}.`);
  };
  ...
  const tadhamonFastStatementHtml = useMemo(() => {
    ...
    const rows = statementRows.map((row) => ({ date: displayStatementDate(row.date), reference: row.operationNumber, description: row.description, credit: row.credit, debit: row.debit, balance: row.balance, highlightColor: fastHighlightColors[row.rowNumber] || quickHighlightConfig.depositColor }));
    const keyword = fastKeyword.trim().toLocaleLowerCase();
    ...
    const fastHighlights = { ...keywordHighlights, ...Object.fromEntries(rows.filter((row) => row.highlightColor && row.highlightColor.toLowerCase() !== quickHighlightConfig.depositColor.toLowerCase()).map((row) => [row.reference, row.highlightColor as string])) };
    return renderTadhamonFastStatementPages(profile, rows, fastHighlights, fastRowsPerPage);
  }, [barcodeSources, documentClient, documentPrintDate, documentPeriodEnd, quickHighlightConfig.depositColor, reportedClosing, reportedTotalCredit, reportedTotalDebit, selectedBank, statementQrSources, statementReference, statementRows, fastHighlightColors, fastKeyword, fastKeywordColor, fastRowsPerPage]);
  ...
  return (
    <div key={selectedBank} className="app-shell" dir="rtl">
      ...
      {activeTab === "fastStatement" && selectedBank === "tadhamon" && <section className="panel print-preview-panel" dir="rtl">
        ...
        <div className="actions"><label className="computed-field"><span>عدد العمليات لكل صفحة / Rows per page</span><input className="transaction-edit-input" type="number" min="1" max="35" step="1" value={fastRowsPerPageInput} onChange={(event) => setFastRowsPerPageInput(event.target.value)} placeholder="35" /></label><button type="button" className="secondary-button" onClick={applyFastRowsPerPage}><RefreshCcw size={17} /> تحديث عدد الصفوف / Update Rows</button><button type="button" className="secondary-button" onClick={() => highlightDepositsWhite()}><RefreshCcw size={17} /> تلوين الإيداعات بلون البنك / Bank Deposit Color</button><label className="computed-field"><span>حد مبلغ الإيداع / Minimum Deposit</span><input className="transaction-edit-input" type="number" min="0" step="0.01" dir="ltr" value={fastMinimumDeposit} onChange={(event) => setFastMinimumDeposit(event.target.value)} placeholder="100.00" /></label><button type="button" className="secondary-button" onClick={() => { const minimum = money(fastMinimumDeposit); if (minimum > 0) highlightDepositsWhite(minimum); }} disabled={!fastMinimumDeposit.trim() || money(fastMinimumDeposit) <= 0}><RefreshCcw size={17} /> تحديث حسب المبلغ / Apply Minimum</button><div className="keyword-color-controls" style={{ display: "flex", gap: "10px", alignItems: "end", flexWrap: "wrap", width: "100%", padding: "12px", border: "1px solid #dbe3ec", borderRadius: "8px", background: "#f8fafc" }}><label className="computed-field"><span>كلمة البحث في الإيداعات / Deposit Keyword</span><input className="transaction-edit-input" value={fastKeyword} onChange={(event) => setFastKeyword(event.target.value)} placeholder="مثال: إيداع أو راتب" /></label><label className="computed-field"><span>لون الوصف المطابق / Match Color</span><select className="transaction-edit-input" value={fastKeywordColor} onChange={(event) => setFastKeywordColor(event.target.value)}><option value="#e5e7eb">أسود / Black</option><option value="#dcfce7">أخضر / Green</option><option value="#dbeafe">أزرق / Blue</option><option value="#fef9c3">أصفر / Yellow</option><option value="#fee2e2">أحمر / Red</option></select></label><button type="button" className="secondary-button" onClick={applyFastKeywordColor} disabled={!fastKeyword.trim()}>تطبيق على الأوصاف المطابقة / Apply</button><small style={{ color: "#475569", width: "100%" }}>يتم تلوين الإيداعات المطابقة فقط؛ السحوبات وباقي العمليات لا تتغير، والخط يبقى أسوداً. / Deposits only; all other rows stay unchanged.</small></div><button type="button" className="preview-button" onClick={() => openPrintWindow(tadhamonFastStatementHtml, `${quickHighlightConfig.label} — Quick Account Statement`)}><FileText size={17} /> معاينة / Preview</button><button type="button" onClick={() => void downloadPdf("accountStatement")}><Printer size={17} /> طباعة / Print</button><button type="button" className="unified-print-button" onClick={() => printDocument("unifiedAll")}><Printer size={17} /> طباعة موحدة / Unified Print</button></div>
        {transactions.length > 0 && <div className="table-wrap"><table><thead><tr><th>Date</th><th>Reference</th><th>Color</th><th>Description</th><th>Debit</th><th>Credit</th><th>Balance</th></tr></thead><tbody>{transactions.map((row) => <tr key={`fast-${row.rowNumber}-${row.operationNumber}`}><td>{displayStatementDate(row.date)}</td><td dir="ltr">{row.operationNumber}</td><td><label className="operation-color-control"><span className="sr-only">لون العملية {row.operationNumber}</span><input aria-label={`Quick highlight ${row.operationNumber}`} type="color" value={fastHighlightColors[row.rowNumber] || quickHighlightConfig.depositColor} onChange={(event) => updateFastHighlight(row.rowNumber, event.target.value)} /><select aria-label={`Quick preset color ${row.operationNumber}`} value={fastHighlightColors[row.rowNumber] || quickHighlightConfig.depositColor} onChange={(event) => updateFastHighlight(row.rowNumber, event.target.value)}><option value="#ffed00">أصفر فاتح</option><option value="#e5e7eb">أسود فاتح</option><option value="#dcfce7">أخضر فاتح</option><option value="#dbeafe">أزرق فاتح</option><option value="#fef9c3">أصفر خفيف</option><option value="#fee2e2">أحمر فاتح</option></select><button type="button" className="secondary-button" title="إرجاع اللون الأصفر" onClick={() => updateFastHighlight(row.rowNumber, quickHighlightConfig.depositColor)}>إعادة</button></label></td><td>{row.description}</td><td>{row.debit ? formatMoney(row.debit) : "—"}</td><td>{row.credit ? formatMoney(row.credit) : "—"}</td><td>{formatMoney(row.balance || 0)}</td></tr>)}</tbody></table></div>}
        <div className="actions"><button type="button" className="unified-print-button" onClick={() => void downloadDocumentPdf("accountStatement", tadhamonFastStatementHtml, client.name)} disabled={downloadingDocument === "accountStatement"}><Printer size={17} /> {downloadingDocument === "accountStatement" ? "جارٍ تجهيز PDF…" : "حفظ الكشف السريع PNG/PDF"}</button><button type="button" className="unified-print-button" onClick={() => void downloadPdf("unifiedAll")} disabled={downloadingDocument === "unifiedAll"}><Printer size={17} /> {downloadingDocument === "unifiedAll" ? "جارٍ تجهيز الحزمة…" : "حفظ الموحد PNG/PDF"}</button></div><div className="document-frame-wrap"><iframe key={`quick-preview-${fastRowsPerPage}-${tadhamonFastStatementHtml.length}`} className="document-frame" title="Tadhamon quick account statement preview" srcDoc={tadhamonFastStatementHtml} /></div>
      </section>}
