...
function AuthenticatedHome({ user, logout }: { user: { name?: string | null; email?: string | null }; logout: () => Promise<void> }) {
  ...
  const printDocument = (kind: PrintDocumentKind) => {
    ...
    if (!openPrintWindow(selected.html, person ? `${person} - ${selected.title}` : selected.title, window, client.branch, documentClient.printDate)) {
  ...
  const downloadPdf = async (kind: PrintDocumentKind) => {
    ...
    try {
      ...
      const previewDocument = previewTitle
        ...
        : undefined;
      const opened = await downloadDocumentPdf(kind, selected.html, client.name, previewDocument, client.branch, documentClient.printDate);
