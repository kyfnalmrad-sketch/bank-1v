...
type QuickLocalSettings = { rowsPerPage?: number; depositColor?: string; keyword?: string; keywordColor?: string };
...
function AuthenticatedHome({ user, logout }: { user: { name?: string | null; email?: string | null }; logout: () => Promise<void> }) {
  ...
  const initialRowsPerPage = Math.max(1, Math.floor(initialQuickSettings.rowsPerPage || 35));
  ...
  useEffect(() => {
    ...
    const rows = Math.max(1, Math.floor(settings.rowsPerPage || 35));
  ...
  useEffect(() => {
    ...
    const settings: QuickLocalSettings = { rowsPerPage: fastRowsPerPage, depositColor: fastDepositColor, keyword: fastKeyword, keywordColor: fastKeywordColor };
    window.localStorage.setItem(quickSettingsStorageKey(selectedBank), JSON.stringify(settings));
  }, [fastDepositColor, fastKeyword, fastKeywordColor, fastRowsPerPage, selectedBank]);
  ...
  useEffect(() => {
    ...
    const restoredRowsPerPage = typeof localRowsPerPage === "number" && Number.isFinite(localRowsPerPage) ? Math.max(1, Math.floor(localRowsPerPage)) : typeof payload.fastRowsPerPage === "number" && Number.isFinite(payload.fastRowsPerPage) ? Math.max(1, Math.floor(payload.fastRowsPerPage)) : 35;
  ...
  const applyFastRowsPerPage = () => {
    ...
    const next = Number.isFinite(requested) && requested > 0 ? Math.floor(requested) : 35;
  ...
  const quickStatementHtml = useMemo(() => {
    ...
    const fastHighlights = { ...keywordHighlights, ...Object.fromEntries(rows.filter((row) => row.highlightColor && row.highlightColor.toLowerCase() !== fastDepositColor.toLowerCase()).map((row) => [row.reference, row.highlightColor as string])) };
  ...
  return (
    <div key={selectedBank} className="app-shell" dir="rtl">
      ...
      {activeTab === "fastStatement" && selectedBank && <section className="panel print-preview-panel" dir="rtl">
        ...
        <div className="actions"><label className="computed-field"><span>عدد العمليات لكل صفحة / Rows per page</span><input className="transaction-edit-input" type="number" min="1"  step="1" value={fastRowsPerPageInput} onChange={(event) => { const value = event.target.value; setFastRowsPerPageInput(value); const parsed = Number(value); if (Number.isFinite(parsed) && parsed >= 1) setFastRowsPerPage(Math.floor(parsed)); }} placeholder="35" /></label><button type="button" className="secondary-button" onClick={applyFastRowsPerPage}><RefreshCcw size={17} /> تحديث عدد الصفوف / Update Rows</button><button type="button" className="secondary-button" onClick={() => highlightDepositsWhite()}><RefreshCcw size={17} /> تطبيق لون الإيداعات / Apply Deposit Color</button><label className="computed-field"><span>لون الإيداعات / Deposit Color</span><input className="transaction-edit-input" aria-label="لون الإيداعات / Deposit Color" type="color" value={fastDepositColor} onChange={(event) => setFastDepositColor(event.target.value)} /></label><label className="computed-field"><span>حد مبلغ الإيداع / Minimum Deposit</span><input className="transaction-edit-input" type="number" min="0" step="0.01" dir="ltr" value={fastMinimumDeposit} onChange={(event) => setFastMinimumDeposit(event.target.value)} placeholder="100.00" /></label><button type="button" className="secondary-button" onClick={() => { const minimum = money(fastMinimumDeposit); if (minimum > 0) highlightDepositsWhite(minimum); }} disabled={!fastMinimumDeposit.trim() || money(fastMinimumDeposit) <= 0}><RefreshCcw size={17} /> تحديث حسب المبلغ / Apply Minimum</button><div className="keyword-color-controls" style={{ display: "flex", gap: "10px", alignItems: "end", flexWrap: "wrap", width: "100%", padding: "12px", border: "1px solid #dbe3ec", borderRadius: "8px", background: "#f8fafc" }}><label className="computed-field"><span>كلمة البحث في الإيداعات / Deposit Keyword</span><input className="transaction-edit-input" value={fastKeyword} onChange={(event) => setFastKeyword(event.target.value)} placeholder="مثال: إيداع أو راتب" /></label><label className="computed-field"><span>لون الوصف المطابق / Match Color</span><input className="transaction-edit-input" aria-label="لون الكلمة المطابقة / Match Color" type="color" value={fastKeywordColor} onChange={(event) => setFastKeywordColor(event.target.value)} /></label><button type="button" className="secondary-button" onClick={applyFastKeywordColor} disabled={!fastKeyword.trim()}>تطبيق على الأوصاف المطابقة / Apply</button><small style={{ color: "#475569", width: "100%" }}>يتم تلوين الإيداعات المطابقة فقط؛ السحوبات وباقي العمليات لا تتغير، والخط يبقى أسوداً. / Deposits only; all other rows stay unchanged.</small></div><button type="button" className="preview-button" onClick={() => openPrintWindow(quickStatementHtml, `${quickHighlightConfig.label} — Quick Account Statement`)}><FileText size={17} /> معاينة / Preview</button><button type="button" onClick={() => void downloadDocumentPdf("accountStatement", quickStatementHtml, selectedBank === "ycb" ? ycbClient.name : client.name)}><Printer size={17} /> طباعة / Print</button><button type="button" className="unified-print-button" onClick={() => printDocument("unifiedAll")}><Printer size={17} /> طباعة موحدة / Unified Print</button></div>
