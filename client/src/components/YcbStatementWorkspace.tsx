...
export default function YcbStatementWorkspace({ profile, transactions, onBack }: Props) {
  ...
  useEffect(() => {
    ...
    Promise.all(pageGroups.map((rows, index) => QRCode.toDataURL(buildYcbStatementQrPayload(buildInput(rows, index + 1)), { width: 420, margin: 2, errorCorrectionLevel: "H", color: { dark: "#2d3192", light: "#ffffff" } }))).then((sources) => { if (!cancelled) setQrSources(sources); }).catch(() => { if (!cancelled) setQrSources([]); });
    try {
