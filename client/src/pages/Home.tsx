...
function AuthenticatedHome({ user, logout }: { user: { name?: string | null; email?: string | null }; logout: () => Promise<void> }) {
  ...
  const colorQuickStatement = () => {
    const next = Object.fromEntries(transactions.map((transaction) => [transaction.rowNumber, fastDepositColor]));
    setFastHighlightColors(next);
    setImportNote(`تم تلوين كشف السريع فقط باللون المختار. لم تتغير ألوان سجل العمليات أو الكشف الرسمي.`);
  };
  ...
  const quickStatementHtml = useMemo(() => {
    ...
    return renderTadhamonFastStatementPages(profile, rows, fastHighlights, 35);
