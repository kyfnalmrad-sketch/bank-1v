...
function closing(profile: TadhamonFastStatementProfile, qrUri: string) {
  return `<div class="closing-row"><div><div class="bottom-date"><div class="date-label">DATE / التاريخ</div><div class="date-value">${esc(profile.issueDate)}</div></div><section class="footer"><div class="identity-block"><div class="name-box"><span>${esc(profile.customerName)}</span><span class="name-ar">${esc(profile.holderNameAr || profile.customerName)}</span></div><div class="total-row"><div class="total-combined">Total Balance: ${money(profile.closingBalance)}</div><div class="time">Time: <span>${esc(profile.statementTime || "—")}</span></div></div></div></section></div><div class="qr-block"><span class="qr-name">${esc(profile.holderNameAr || profile.customerName)}</span>${qrUri ? `<img class="qr-image" alt="QR code for account holder" src="${esc(qrUri)}">` : ""}</div></div>`;
}
