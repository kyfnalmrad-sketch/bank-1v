...
export function YcbCertificateWorkspace({ client, onChange, onBack }: Props) {
  ...
  return <main className="bank-workspace" dir="rtl">
    <div className="bank-workspace-heading"><div><p className="eyebrow">YEMEN COMMERCIAL BANK</p><h2>بيانات وإصدار شهادة بنك اليمن التجاري</h2><p className="hint">نفس تصميم مدخلات النظام الأصلي، مع حذف رقم المميز لأنه غير مستخدم في بنك اليمن التجاري.</p></div><button type="button" className="secondary-button" onClick={onBack}><ArrowRight size={16} /> اختيار بنك آخر</button></div>
    <section className="panel bank-form-panel"><div className="panel-heading"><div><h2>بيانات العميل والحساب / Customer & Account Details</h2><p className="hint">هذه البيانات خاصة ببنك اليمن التجاري وترتبط مباشرة بالقالب الظاهر أدناه.</p></div><FileText size={26} className="heading-icon" /></div><div className="grid">
      ...
      <label>الرصيد / Closing balance<input dir="ltr" value={client.opening} onChange={(e) => onChange("opening", e.target.value)} /></label>
