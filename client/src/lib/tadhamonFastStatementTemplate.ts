...
function row(item: YcbStatementTransaction | null, highlights: Record<string, string>) {
  if (!item) return `<tr class="empty-row">${Array.from({ length: 7 }, () => "<td>&nbsp;</td>").join("")}</tr>`;
...
export function renderTadhamonFastStatement(profile: TadhamonFastStatementProfile, transactions: YcbStatementTransaction[], highlights: Record<string, string> = {}, rowsPerPage = 35) {
  ...
  const pageMarkup = Array.from({ length: pages }, (_, pageIndex) => {
    ...
    const paddedRows: Array<YcbStatementTransaction | null> = [...pageRows, ...Array.from({ length: Math.max(0, safeRowsPerPage - pageRows.length) }, () => null)];
    const isLast = pageIndex === pages - 1;
    return `<main class="page">${pageIndex === 0 ? header(profile) : ""}<table class="statement"><colgroup><col class="date"><col class="ref"><col class="desc"><col class="debit"><col class="credit"><col class="balance"><col class="remarks"></colgroup>${tableHeader()}<tbody>${paddedRows.map((item) => row(item, highlights)).join("")}</tbody></table>${isLast ? closing(profile, profile.qrUri || "") : ""}</main>`;
  }).join("");
