  ...
  return (
    <div className="app-shell" dir="rtl">
      ...
      {activeTab === "review" && <section className="panel review-panel">
        ...
        <div className="actions document-actions"><button type="button" className="secondary-button" onClick={() => void saveStatementHistory()} disabled={createHistoryMutation.isPending || updateHistoryMutation.isPending}><Database size={17} /> {editingHistoryId ? "Update Saved Statement" : "Save to Statement History"}</button><button type="button" onClick={() => selectedBank === "ycb" ? setShowYcbStatement(true) : setReviewPreview("accountStatus")}><FileText size={17} /> {selectedBank === "ycb" ? "View Approved YCB Statement" : "View Account Status Statement"}</button><button type="button" className="preview-button" onClick={() => selectedBank === "ycb" ? setShowYcbStatement(true) : setReviewPreview("accountStatement")}><FileText size={17} /> View Account Statement</button><button type="button" className="unified-print-button" onClick={() => selectedBank === "ycb" ? setShowYcbStatement(true) : printDocument("unified")}><Printer size={17} /> {selectedBank === "ycb" ? "Open Approved YCB Statement" : "Print Unified PDF (Statement + Status)"}</button><button type="button" onClick={() => printDocument("accountStatus")}><Printer size={17} /> {selectedBank === "ycb" ? "Print Approved YCB PDF" : "Print Account Status / Save PDF"}</button><button type="button" className="preview-button" onClick={() => selectedBank === "ycb" ? setShowYcbStatement(true) : printDocument("accountStatement")}><Printer size={17} /> Print Account Statement / Save PDF</button><button type="button" disabled={downloadingDocument === "accountStatus"} onClick={() => void downloadPdf("accountStatus")}><Download size={17} /> {downloadingDocument === "accountStatus" ? "Opening print…" : selectedBank === "ycb" ? "Print / Save Approved YCB PDF" : "Print / Save Account Status PDF"}</button><button type="button" className="preview-button" disabled={selectedBank === "ycb" || downloadingDocument === "accountStatement"} onClick={() => void downloadPdf("accountStatement")}><Download size={17} /> {downloadingDocument === "accountStatement" ? "Opening print…" : "Print / Save Account Statement PDF"}</button><button type="button" className="secondary-button" onClick={downloadSessionJson}><RefreshCcw size={17} /> Download JSON Session</button></div>
