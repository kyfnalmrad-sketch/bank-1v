...
function AuthenticatedHome({ user, logout }: { user: { name?: string | null; email?: string | null }; logout: () => Promise<void> }) {
  ...
  const quickStatementHtml = useMemo(() => {
    ...
    const rows = statementRows.map((row) => ({ date: displayStatementDate(row.date), reference: row.operationNumber, description: row.description, credit: row.credit, debit: row.debit, balance: row.balance, highlightColor: fastHighlightColors[row.rowNumber] || ((row.credit || 0) > 0 ? fastDepositColor : "") }));
    const keyword = fastKeyword.trim().toLocaleLowerCase();
    ...
    const fastHighlights = { ...Object.fromEntries(rows.filter((row) => (row.credit || 0) > 0 && row.highlightColor).map((row) => [row.reference, row.highlightColor as string])), ...keywordHighlights };
    return renderTadhamonFastStatementPages(profile, rows, fastHighlights, fastRowsPerPage);
  }, [barcodeSources, documentClient, documentPrintDate, documentPeriodEnd, fastDepositColor, quickHighlightConfig.label, reportedClosing, reportedTotalCredit, reportedTotalDebit, selectedBank, statementQrSources, statementReference, statementRows, fastHighlightColors, fastKeyword, fastKeywordColor, fastRowsPerPage, ycbClient.customerSince, ycbStatementProfile]);
